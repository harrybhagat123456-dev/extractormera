...
def process_topic(subject_id, topic_id, batch_id, headers, token, Key, iv, subject_name=""):
            ...
            try:
                ...
                for item in res6_list:
                    ...
                    if url and not url.endswith('.ws'):
                        ...
                        item_date = extract_date(item); date_str = f"{item_date} " if item_date else ""; prefix = f"[{subject_name}] " if subject_name else ""; cc = f'{prefix}{date_str}{title}: {url}'
